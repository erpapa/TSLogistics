...
@interface TSLExpressStationCell()
...
@implementation TSLExpressStationCell

+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    static NSString *ID = @"ExpressStationCell";
    TSLExpressStationCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil){
        cell = [[TSLExpressStationCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        // 3.网点类别
        UILabel *ESTypeLabel = [[UILabel alloc] initWithFrame:CGRectMake(nameX, kRowHeight + 1, 17 * 4.5, kRowHeight)];
        ESTypeLabel.text = @"网点类别:";
        ESTypeLabel.textColor = [UIColor grayColor];
        self.ESType = [[UILabel alloc] initWithFrame:CGRectMake(nameX, kRowHeight + 1, kLineWidth * 0.5, kRowHeight)];
        // 4.上门取件
        UILabel *ShangMenLabel = [[UILabel alloc] initWithFrame:CGRectMake(kLineWidth * 0.5, kRowHeight + 1, 17 * 4.5, kRowHeight)];
        ShangMenLabel.text = @"上门取件:";
        ShangMenLabel.textColor = [UIColor grayColor];
        self.ShangMen = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(ShangMenLabel.frame), kRowHeight + 1, ScreenW - CGRectGetMaxX(ShangMenLabel.frame), kRowHeight)];
        // 5.分割线
        ...
        // 6.地址
        self.ESAddress = [[UILabel alloc] initWithFrame:CGRectMake(nameX, (kRowHeight + 1) * 2, kLineWidth, kRowHeight)];
        self.ESAddress.textColor = [UIColor orangeColor];
        // 7.分割线
        ...
        [self.contentView addSubview:self.ESType];
        [self.contentView addSubview:ShangMenLabel];
        [self.contentView addSubview:self.ShangMen];
        [self.contentView addSubview:line1];
        [self.contentView addSubview:self.ESAddress];
...
- (void)setInfo:(TSLExpressStationInfo *)info
{
    [super setInfo:info];
