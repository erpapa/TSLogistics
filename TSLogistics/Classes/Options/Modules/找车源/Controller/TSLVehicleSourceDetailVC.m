...
@interface TSLVehicleSourceDetailVC ()
...
@property (strong, nonatomic) UILabel *WeightRemaind;
...
@implementation TSLVehicleSourceDetailVC
...
- (void)setupContentView
{
    ...
    // 2.1.车源信息
    UIButton *VehicleInfo = [self buttonWithTitle:@"车源信息" image:@"carBlueImage"];
    // 2.2.分割线
    UIView *line2 = [self lineWithOrigin:CGPointMake(0, kHeight)];
    // 2.3.车牌号
    UILabel *NameCHNLabel = [self labelWithTitle:@"车牌号:" origin:CGPointMake(leftLabelX, kHeight + 1)];
    self.NameCHN = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(NameCHNLabel.frame), kHeight + 1, rightlabelX - CGRectGetMaxX(NameCHNLabel.frame), kHeight)];
    // 2.4.车辆类型
    UILabel *VehicleTypeLabel = [self labelWithTitle:@"车辆类型:" origin:CGPointMake(rightlabelX, kHeight + 1)];
    self.VehicleType = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(VehicleTypeLabel.frame), kHeight + 1, lineWidth - CGRectGetMaxX(VehicleTypeLabel.frame), kHeight)];
    // 2.5.车辆载重
    ...
    // 2.6.车长
    UILabel *LengthLable = [self labelWithTitle:@"车长要求:" origin:CGPointMake(rightlabelX, kHeight * 2 + 1)];
    self.Length = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(LengthLable.frame), kHeight * 2 + 1, lineWidth - CGRectGetMaxX(LengthLable.frame), kHeight)];
    ...
    [bgView2 addSubview:VehicleTypeLabel];
    [bgView2 addSubview:self.VehicleType];
    [bgView2 addSubview:WeightLabel];
    [bgView2 addSubview:self.Weight];
    [bgView2 addSubview:LengthLable];
    [bgView2 addSubview:self.Length];
    [self.scrollView addSubview:bgView2];
    
    // 3.1.货物要求
    UIButton *goodsSource = [self buttonWithTitle:@"货物要求" image:@"goodsImage"];
    // 3.2.分割线
    UIView *line3 = [self lineWithOrigin:CGPointMake(0, kHeight)];
    // 3.3.货物类型
    UILabel *TransGoodsTypeLabel = [self labelWithTitle:@"货物类型:" origin:CGPointMake(leftLabelX,kHeight + 1)];
    self.TransGoodsType = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(TransGoodsTypeLabel.frame), kHeight + 1, rightlabelX - CGRectGetMaxX(TransGoodsTypeLabel.frame), kHeight)];
    // 3.4.运输类型
    ...
    // 3.5.运输价格
    UILabel *PriceLabel = [self labelWithTitle:@"运输价格:" origin:CGPointMake(leftLabelX, kHeight * 2 + 1)];
    self.Price = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(PriceLabel.frame), kHeight * 2 + 1, rightlabelX - CGRectGetMaxX(PriceLabel.frame), kHeight)];
    // 3.6.剩余运输量
    UILabel *WeightRemaindLabel = [self labelWithTitle:@"剩余运输量:" origin:CGPointMake(rightlabelX, kHeight * 2 + 1)];
    self.WeightRemaind = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(WeightRemaindLabel.frame), kHeight * 2 + 1, lineWidth - CGRectGetMaxX(WeightRemaindLabel.frame), kHeight)];
    ...
    [bgView3 addSubview:TransGoodsTypeLabel];
    [bgView3 addSubview:self.TransGoodsType];
    [bgView3 addSubview:PriceTypeLabel];
    [bgView3 addSubview:self.PriceType];
    [bgView3 addSubview:PriceLabel];
    [bgView3 addSubview:self.Price];
    [bgView3 addSubview:WeightRemaindLabel];
    [bgView3 addSubview:self.WeightRemaind];
...
- (void)loadInfo
{
    ...
    if (self.Identifier) {
        ...
        [TSLHttpTool POST:[NSString stringWithFormat:@"%@%@", TSLAPI_PREFIX, TSLAPI_VehicleSource] parameters:param success:^(id json) {
            NSArray *infoArray = [TSLVehicleSourceInfo objectArrayWithKeyValuesArray:json];
 ...
 *  赋值
 */
- (void)loadInfo:(TSLVehicleSourceInfo *)info
{
    ...
    self.WeightRemaind.text = [NSString stringWithFormat:@"%gt", info.WeightRemaind];
...
{
    if (success) { // 收藏成功，置1
        TSLVehicleSourceInfo *info = (TSLVehicleSourceInfo *)self.info;
        info.iscollected = @"1";
    }
}
