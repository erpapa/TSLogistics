...
@property (strong, nonatomic) UILabel *RequirementVehicleType;
...
@implementation TSLGoodsSourceCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    // dequeue : 出列 (查找)
    TSLGoodsSourceCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    // 2.如果没有可循环利用的cell
    if (cell == nil){
        cell = [[TSLGoodsSourceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        // 5.货物名称
        self.NameCHN = [[UILabel alloc] initWithFrame:CGRectMake(nameX, (kRowHeight + 1) * 2, kLineWidth * 0.4, kRowHeight)];
        // 6.货物重量
        self.GoodsWeight = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.NameCHN.frame), (kRowHeight + 1) * 2, kLineWidth * 0.3, kRowHeight)];
        self.GoodsWeight.textColor = [UIColor orangeColor];
        self.GoodsWeight.textAlignment = NSTextAlignmentCenter;
        // 7.货车类型
        self.RequirementVehicleType = [[UILabel alloc] initWithFrame:CGRectMake(timeX, (kRowHeight + 1) * 2, timeWidth, kRowHeight)];
        self.RequirementVehicleType.textColor = [UIColor orangeColor];
        // 8.分割线
        ...
        [self.contentView addSubview:self.GoodsWeight];
        [self.contentView addSubview:self.RequirementVehicleType];
...
- (void)setInfo:(TSLGoodsSourceInfo *)info
{
    [super setInfo:info];
    ...
    self.RequirementVehicleType.text = info.RequirementVehicleTypeString;
