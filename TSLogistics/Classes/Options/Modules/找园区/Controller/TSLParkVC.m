...
@implementation TSLParkVC
- (void)viewDidLoad {
    [super viewDidLoad];
    self.tableView.rowHeight = TSLParkCellHeight;
...
- (void)loadNewData
{
    ...
    [TSLHttpTool POST:[NSString stringWithFormat:@"%@%@", TSLAPI_PREFIX, TSLAPI_Park] parameters:param success:^(id json) {
        ...
        TSLParkInfo *firstInfo = [self.infoArray firstObject];
        if ([[firstDict valueForKey:@"Identifier"] integerValue] != firstInfo.Identifier) {
            self.infoArray = [TSLParkInfo objectArrayWithKeyValuesArray:json];
...
- (void)loadMoreData
{
    ...
    [TSLHttpTool POST:[NSString stringWithFormat:@"%@%@", TSLAPI_PREFIX, TSLAPI_Park] parameters:param success:^(id json) {
        NSArray *array = [TSLParkInfo objectArrayWithKeyValuesArray:json];
...
{
    TSLParkCell *cell = [TSLParkCell cellWithTableView:tableView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TSLParkDetailVC *detailVC = [[TSLParkDetailVC alloc] init];
