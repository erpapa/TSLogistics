...
#import "TSLWlTerminalCell.h"

@interface TSLWlTerminalCell()
...
@implementation TSLWlTerminalCell

+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    static NSString *ID = @"ExpressStationCell";
    TSLWlTerminalCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil){
        cell = [[TSLWlTerminalCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        // 3.类别
        self.TypeId = [[UILabel alloc] initWithFrame:CGRectMake(nameX, kRowHeight + 1, kLineWidth * 0.5, kRowHeight)];
        // 4.所属区域
        self.TdRegionName = [[UILabel alloc] initWithFrame:CGRectMake(kLineWidth * 0.5, kRowHeight + 1, kLineWidth * 0.5, kRowHeight)];
        // 5.分割线
        ...
        // 6.服务范围
        self.TdScope = [[UILabel alloc] initWithFrame:CGRectMake(nameX, (kRowHeight + 1) * 3, kLineWidth, kRowHeight)];
        ...
        // 12.地址
        self.TdAddress = [[UILabel alloc] initWithFrame:CGRectMake(nameX, (kRowHeight + 1) * 4, kLineWidth, kRowHeight)];
        // 13.添加子控件
        [self.contentView addSubview:self.NameCHN];
        [self.contentView addSubview:line0];
        [self.contentView addSubview:self.TypeId];
        [self.contentView addSubview:self.TdRegionName];
        ...
        [self.contentView addSubview:self.TdScope];
        [self.contentView addSubview:line3];
        [self.contentView addSubview:self.TdAddress];
    }
    return self;
}

- (void)setInfo:(TSLWlTerminalInfo *)info
{
    [super setInfo:info];
    ...
    self.TdScope.text = [NSString stringWithFormat:@"服务范围:%@",info.TdScope];
    self.TdAddress.text = [NSString stringWithFormat:@"地址:%@",info.TdAddress];
}
