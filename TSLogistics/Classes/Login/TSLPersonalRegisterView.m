...
@interface TSLPersonalRegisterView()<UITextFieldDelegate>
...
@end
@implementation TSLPersonalRegisterView
...
- (void)setupContentView
{
    ...
    self.compellation.placeholder = @"姓名";
    ...
    UIButton *areaButton = [[UIButton alloc] initWithFrame:CGRectMake(x, height * 5 + 60, width, height)];
    ...
    UIButton *registerButton = [[UIButton alloc] initWithFrame:CGRectMake(x, height * 6 + 70, width, height)];
    registerButton.backgroundColor = [UIColor orangeColor];
    [registerButton setTitle:@"注册" forState:UIControlStateNormal];
    [registerButton addTarget:self action:@selector(registerButtonClick) forControlEvents:UIControlEventTouchUpInside];
    
    UILabel *aboutLabel = [[UILabel alloc] initWithFrame:CGRectMake(x, height * 7 + 80, width, 20)];
    aboutLabel.text = @"点击注册即表示已阅读并同意";
    aboutLabel.font = [UIFont systemFontOfSize:14];
    aboutLabel.textAlignment = NSTextAlignmentCenter;
    
    UIButton *readme = [[UIButton alloc] initWithFrame:CGRectMake(x, height * 7 + 100, width, 20)];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    CGFloat h = CGRectGetMaxY(frame) + 105 - 316;
    if (h > 0) {
